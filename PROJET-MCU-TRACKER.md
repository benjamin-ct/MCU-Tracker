...
- **GitHub** : repo `benjamin-ct/MCU-Tracker`, branche `main`. Contient `index.html` + `css/` + `js/` + `functions/` + ce fichier + `README.md`. Poussé directement par Claude Code (accès en écriture) — le connecteur GitHub de Claude.ai classique reste lecture seule par design, différent de l'intégration Claude Code.
...
- **Cloudflare Pages** : site en production sur [mcuwatchtimeline.com](https://mcuwatchtimeline.com) (domaine perso de l'utilisateur), géré par l'utilisateur directement. Utilise `functions/api/tmdb/[[path]].js` (voir "Proxy TMDB" ci-dessous) — nécessite que la variable d'environnement `TMDB_KEY` soit configurée côté Cloudflare (Pages → Settings → Environment variables) pour que les affiches marchent automatiquement pour les visiteurs.
...
## Proxy TMDB (`functions/api/tmdb/[[path]].js`)
...
- **Pourquoi une Pages Function et pas juste une variable d'environnement lue côté client** : une variable d'environnement injectée dans du JS qui tourne dans le navigateur reste visible par n'importe qui (onglet réseau, "voir le code source") — ce n'est pas un vrai secret. Une Cloudflare Pages Function tourne côté serveur : elle seule lit `env.TMDB_KEY`, et ne renvoie au client que `{poster: "/xxx.jpg"|null}`, jamais la clé ni la réponse TMDB complète.
- **Route** : `GET /api/tmdb/<movie|tv>/<id>` (routing par segments `[[path]]`, catch-all Cloudflare Pages Functions). Réponses : `200 {poster}` (y compris `poster:null` si TMDB répond 404 — ce n'est pas une panne, juste "pas d'affiche"), `501` si `TMDB_KEY` n'est pas configurée, `400` si type/id invalide, `502` si TMDB répond une autre erreur ou si le réseau échoue.
- **Client** (`js/modals.js`, `fetchPosterViaProxy()`/`fetchRealPoster()`) : essaie d'abord ce proxy same-origin, sans clé. Si la réponse n'est pas `res.ok` (route absente — `file://`, hébergeur sans Pages Functions, clé serveur pas configurée — ou erreur réseau), retombe sur la clé TMDB personnelle si l'utilisateur en a collé une dans l'app, puis sur l'affiche générée (dégradé + initiales) en dernier recours — jamais de crash, même chaîne de repli que le reste de l'app (voir décision d'architecture #5).
- **Limite connue** : cette fonction ne tourne que sur Cloudflare Pages (dossier `functions/` auto-détecté). Un `index.html` ouvert en `file://`, ou hébergé ailleurs (Netlify sans fonction équivalente), continue de fonctionner exactement comme avant — juste sans le bénéfice du proxy, via les mêmes replis.
